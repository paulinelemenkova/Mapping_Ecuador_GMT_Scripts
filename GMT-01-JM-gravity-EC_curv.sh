...
gmt img2grd curv_27.1.img -R277/285/-5/1.5 -Ggrav_v_EC.grd -T1 -I1 -E -S0.1 -V
gdalinfo grav_v_EC.grd -stats
# Minimum=-728.471, Maximum=571.765, Mean=-0.050, StdDev=60.888

# Generate a color palette table from grid
# gmt makecpt --help
gmt makecpt -Crainbow -T-150/250 > colors.cpt

# Generate a file
ps=Grav_EC_v.ps
gmt grdimage grav_v_EC.grd -Ccolors.cpt -R277/285/-5/1.5 -JM6.5i -P -I+a15+ne0.75 -Xc -K > $ps
# Add isolines
gmt grdcontour grav_v_EC.grd -R -J -C200 -A400 -Wthinner -O -K >> $ps
# Add grid
gmt psbasemap -R -J \
    --MAP_FRAME_AXES=WEsN \
    ...
    -B+t"Vertical gravity gradient grid for Ecuador" -O -K >> $ps
# Add legend
...
gmt psconvert Grav_EC_v.ps -A0.5c -E720 -Tj -Z
